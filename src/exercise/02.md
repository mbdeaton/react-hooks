...
#### Key Takeaway
The `useEffect()` function is passed a function that is called every time
the component renders. Specify this function to execute any side-effects.
...
function Setter({initX = ''}) {
  ...
  React.useEffect(() => window.localStorage.setItem('x', x))
  function handleChange(event) { setX(event.target.value) }
  return (
    <>
...
function useLocalStorageState(key, defaultVal='') {
  const [state, setState] = React.useState(window.localStorage.getItem(key) ?? defaultVal)
