...
Wherever your different pieces of state overlap logically, prefer to manage a
minimal set of pieces of state, and derive the other bits. This best practice
avoids a large opportunity for bugs due to mismatched state.

#### Tips

Best practice for updating state is to copy and modify the copy, rather than
mutate state directly. (In fact I think the latter is incorrect with useState.)
For example, given `[x, setX] = useState(obj)`, instead of `x.prop = foo`, do:
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=04%3A%20useState%3A%20tic%20tac%20toe&em=brett.deaton%40gmail.com).
